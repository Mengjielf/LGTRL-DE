...
for i in range(times):
    ...
    x_test, y_test, demographic_test, los_test = load_5fold_mimic(config.data_path + str(i + 1) + "fold_test_x.npy",
                                                                  ...
                                                                  config.data_path + str(i + 1) + "fold_test_demo.npy",
                                                                  config.data_path + str(i + 1) + "fold_test_los.npy")
    emb_size = 128
    trans_emb_size = emb_size
    demo_emb_size = emb_size
    atten_emb_size = emb_size
    d_inner_hid = emb_size * 2
    gru_units = int(emb_size / 2)

    filepath = config.model_dir
    filename = "lgtrl_de_" + str(i + 1) + ".h5"
    modelpath = filepath + filename

    model = LGTRL_DE(time_steps, 4, input_dims, emb_size, trans_emb_size, demo_emb_size, d_inner_hid, gru_units,output_dim)
